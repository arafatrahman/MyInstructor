...
// --- UPDATED: Instructor now listens for notifications (cancellations) ---
...
struct InstructorDashboardView: View {
    ...
    // --- NEW: Inject NotificationManager ---
    ...
    // Combined notification count (Requests + Alerts)
    var notificationCount: Int {
        let unreadAlerts = notificationManager.notifications.filter { !$0.isRead }.count
        // Requests are fetched in 'fetchData' but we can assume they contribute if stored.
        // For simplicity, we'll rely on the local state 'pendingRequestCount' if we had it,
        // but here we'll just use the notifications + we'll fetch requests count.
    ...
    var body: some View {
        NavigationView {
            ...
            .task {
                ...
                notificationManager.listenForNotifications(for: instructorID) // --- START LISTENING ---
                await fetchData()
            }
...
// ... (Rest of Subviews: StudentQuickOverviewSheet, StudentCardRow, NextLessonContent, etc. remain unchanged) ...
        ...
        NavigationView {
            ZStack {
                ...
                VStack(spacing: 0) {
                    ...
                    }.padding(10).background(Color(.secondarySystemGroupedBackground)).cornerRadius(10).padding(.horizontal).padding(.vertical, 10)
            ...
            .toolbar {
                ToolbarItem(placement: .navigationBarLeading) { Button(action: { dismiss() }) { HStack(spacing: 5) { Image(systemName: "chevron.left"); Text("Back") }.foregroundColor(.textDark) } }
                ToolbarItem(placement: .navigationBarTrailing) { Button(action: { isAddingStudent = true }) { Image(systemName: "plus").font(.headline).foregroundColor(.primaryBlue) } }
            }
            .task { await loadStudents() }
            .sheet(isPresented: $isAddingStudent) { OfflineStudentFormView(studentToEdit: nil, onStudentAdded: { Task { await loadStudents() } }) }
        }
    }
